...
import net.yanzm.coroutineandroidworkshop.Variant.*
    ...
    private fun load() {
        ...
        launch {
            ...
            val list = withContext(Dispatchers.Default) {
                ...
                try {
                    val response = service.listOrgRepos(req.org).execute()
...
//}
    ...
    private fun doLoad() {
        ...
        when (selectedVariant()) {
            ...
            COROUTINE -> { // Using coroutines
                //coroutine 関数。coroutineを開始する
                //CoroutineScopeをextendsしてる
                launch {
            ...
            FUTURE -> {
                //Futureを使いたかったらcoroutine-jdk8をいれる
    ...
    private fun Job.updateCancelJob() {
        ...
        launch {
            //親のjobが全部終わるまでjoinで待つ
            //suspend関数なのでlaunch関数で呼ぶ
            join()
            updateEnabled(true)
